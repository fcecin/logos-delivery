...
class StepsMessaging(StepsCommon, StepsMetrics):
    ...
    node1 is a core node running the Store service; node2 is a core node that
    connects to node1 and uses it as its Store peer, so node2's sends get the
    "sent" confirmation. Both run --entry-layer=messaging with autosharding
    (one shard), which the content-topic API needs.

    The observation endpoints are evict-after-poll, so the collect_* helpers
    accumulate across polls instead of reading once.
    """
    ...
    def collect_received(self, node, count, timeout=30):
        """Polls the received endpoint (each poll clears it) until `count` records were collected."""
        records = []
    ...
    def collect_send_kinds(self, node, request_id, kinds, timeout=30):
        """Polls the per-request send events (each poll clears them) until every kind in `kinds` was seen.

        A 404 means nothing is buffered yet for the id and is polled through. The status is
        matched at the start of the client's message, since the URL in it carries the random
        REST port and the request id, which can contain any digits.
        """
        seen = set()
        deadline = time() + timeout
    ...
    def collect_sent_request_ids(self, node, count, timeout=60):
        """Polls all send events (each poll clears them) until `count` request ids carry a "sent" event."""
        sent = set()
        deadline = time() + timeout
    ...
    def wait_for_live_received_count(self, node, count, timeout=30):
        """Waits until the node's metrics report `count` live deliveries; unlike the REST poll, reading them evicts nothing."""
