...
proc mixRequired*(self: MessagingClientConf): bool =
  ## The one rule: a level above `None` requires the mix protocol on the node.
...
proc applyAnonymityLevel*(
    self: MessagingClientConf, kernel: var WakuNodeConf
): ConfResult[void] =
  ## Applies the kernel side of the anonymity level to `kernel`: a level that
  ## requires mix mounts it. A kernel record that sets `mix: false` next to
  ## such a level is an error, and so is an IPv6 listen address: the mix pool
  ## and the return paths take IPv4 addresses only, and with an IPv6 bind mix
  ## mounts and each mixed attempt fails. Each input route (the structured
  ## and the flat JSON shapes, the Nim API) calls this, and the constructor
  ## calls it again for a kernel record made by hand. The call is idempotent.
  if not self.mixRequired():
    return ok()
  if kernel.mix == Opt.some(false):
    return err(
      "anonymityLevel=" & $self.anonymityLevel.get() & " needs mix, but mix=false was set"
    )
  if kernel.listenAddress.family == IpAddressFamily.IPv6:
    return err(
