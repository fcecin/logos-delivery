...
proc parseFlatConf(
  ...
  # The messaging configuration owns `anonymityLevel`, and `toWakuNodeConf`
  # applies its kernel side for the structured shape. The flat shape builds
  # its kernel record here, so the same rule runs here.
  ?messaging.applyAnonymityLevel(kernel)
