...
type AnonymityLevel* {.pure.} = enum
  ## How a send may use Mix. A message sent over Mix is complete when the exit
  ## node's reply arrives, and is never confirmed against a store node: that
  ## query would name the message, in clear, from this node's own address.
