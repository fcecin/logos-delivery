...
export MixSendOutcome, MixSendCertainty, MixSendReason, requestLeft
  ## The typed outcome of a mix attempt, for the messaging layer, which does
  ## not import node internals.
...
proc lightpushPublishViaMix*(
  ...
  ## Publishes `message` through mix to an exit node for `shard`. `avoid` is
  ## the exit node of a failed attempt: the selection skips it when a
  ## different exit node exists. The outcome names the exit node it used and
  ## says, in typed fields, what the caller may conclude about the message.
  let peer = self.selectMixLightpushPeer(shard, avoid).valueOr:
    return MixSendOutcome(
      certainty: MixSendCertainty.NotSent,
      reason: MixSendReason.NoExitNode,
      answer: lightpushResultServiceUnavailable(
        "no mix-capable lightpush peer available for shard"
      ),
    )
  try:
    return await self.node.lightpushPublishOverMix(shard, message, peer)
  except CancelledError as exc:
    raise exc
  except CatchableError as e:
    # An error that no layer below classified. Whether the request left this
    # node is not known, so the outcome says that it may have.
    return MixSendOutcome(
      certainty: MixSendCertainty.MaybeSent,
      reason: MixSendReason.ReplyUnreadable,
      exit: Opt.some(peer.peerId),
      answer: lightpushResultInternalError(e.msg),
    )
