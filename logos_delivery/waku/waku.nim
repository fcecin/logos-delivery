...
const DefaultEnrReconcileInterval* = 5.seconds
  ## nim-eth has no hook on its own record writes, so the node polls. Until
  ## the next tick, MyENR can be behind a discovery update, and the live
  ## record can pair the learned host with the previous `tcp`.
...
type Waku* = ref object ## Implements `KernelApi` (ops in `waku/api/*`).
  ...
  enrReconcileInterval*: Duration = DefaultEnrReconcileInterval
    ## Tests shorten it to watch the loop. Production keeps the default.
...
proc refreshEnrAddrs*(
  ...
  ## Write the announced addresses a peer can use from outside into the ENR.
...
proc reconcileEnrAddrs*(
  ...
  ## discv5 writes the host it learned from its peers into its own record,
  ## with the `tcp` the record had, and tells nobody. When the live record
  ## moved away from the node's copy, write the addresses again: `tcp` then
  ## fits the learned host, and the copy follows. True when it wrote.
...
proc updateWaku(waku: Waku): Future[Result[void, string]] {.async.} =
  ...
  if not waku.wakuDiscv5.isNil():
    ...
    ## record gave discv5 its idea of its own address. The rebuilt record
    ## replaces that too, and discv5 learns its address from its peers.
