...
proc discv5NatRenewLoop(
  ...
  ## Re-request the discv5 udp mapping before its finite lease expires
  ## (`NatUdpLeaseSeconds`, one hour; renewal at the same 30-minute cadence
  ## libp2p renews the switch transports). Renewal asks for the external
  ## port the gateway originally granted, so a stable mapping renews
  ## silently. If the gateway ever grants a different port, that is warned
  ## once and tracked; the ENR keeps the original port until the next start
  ## (rebuilding the ENR mid-flight is deliberately out of scope here).
    ...
    while true:
      ...
      if mapped.isErr():
        ...
        warn "discv5 NAT mapping renewed on a different external port; " &
          "the ENR keeps the original until restart",
  ...
  if conf.discv5Conf.isSome():
    ...
    if conf.endpointConf.natStrategy.kind in {NatAny, NatUpnp, NatPmp}:
      try:
        ...
        if mapped.isOk():
          ## discv5Conf.udpPort feeds the ENR rebuild in updateWaku below, so
          ## the granted external port lands in the ENR. Note the overwrite:
          ## from here on this conf field holds the *external* port; the
          ## internal bind port remains in waku.node.ports.discv5Udp and
