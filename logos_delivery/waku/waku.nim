...
proc updateEnr(waku: Waku): Future[Result[void, string]] {.async.} =
  ...
  waku.node.baseAnnouncedAddresses = netConf.announcedAddresses
...
proc discv5NatRenewLoop(
  ...
  ## port the gateway granted. A different granted port is warned and
  ## tracked; the ENR keeps the original until the next start.
