      ...
      of RestRequestError.NotFound:
        ...
        let rootPath =
          if len(paths) > 1:
            ...
            ""
        # Explicit: chronos would default the type to text/html.
...
proc markRestApiInstalled*(rootPath: string) =
  ## Clears the "not available" answer for `rootPath` (e.g. "messaging") once
  ## its endpoints are mounted. The table is process-global (one per thread),
  ## so with several REST servers in one process the last one to mount wins.
...
proc markRestApiNotInstalled*(rootPath: string, reason: string) =
  ## Sets the answer for a root whose endpoints will not be mounted, e.g. the
  ## `--entry-layer` hint for `/messaging` on a kernel-only node.
  if not restServerNotInstalledTab.isNil():
