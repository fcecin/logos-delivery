...
proc resolveMixNodes(
  ...
  ##
  ## An entry that cannot be resolved is dropped with a warning rather than
  ## failing the mount: one unreachable name should not cost the node every
  ## other mix node it was given, nor its own start.
  var
  ...
  try:
    ...
    if not (
      ...
      warn "Timed out resolving mix node names",
        names = pending.len, timeout = MixNodeResolveTimeout
  finally:
    for fut in futures:
      if not fut.finished():
  ...
  for i, mixnode in pending:
    ...
    if fut.cancelled():
      warn "Gave up resolving a mix node name, skipping it",
        multiAddr = mixnode.multiAddr
      continue
    ...
    let addresses =
      try:
        ...
        warn "Failed to resolve a mix node name, skipping it",
          multiAddr = mixnode.multiAddr, error = exc.msg
        continue

    if addresses.len == 0:
      warn "A mix node name resolved to no address, skipping it",
        multiAddr = mixnode.multiAddr
      continue
