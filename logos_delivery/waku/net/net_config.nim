...
func isWildcard*(ip: IpAddress): bool =
  ## The bind-any host, in each of its spellings, which no peer can dial.
  case ip.family
...
proc init*(
  ...
  ## The ENR scalars name an endpoint a peer can use. A wildcard host and
  ## port 0 are bind-time placeholders, whatever their source (an external
  ## ip comes with the bind port as its port), so the record leaves them out
  ## until the rebuild and the refresh at start fill them.
