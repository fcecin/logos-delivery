...
proc closeProbeLater(mapper: PortMapper, discoveryTimeout: Duration) =
  ## Close the probe mapper later and detached, so a probe worker
  ## inside its C call adds no delay to node start. The grace period
  ## outlasts the worker's retry rounds.
