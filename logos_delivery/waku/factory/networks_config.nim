...
type NetworkPresetConf* = object
  ...
  mix*: bool
  mixnodes*: seq[string]
    ## Mix bootstrap nodes, `multiaddr:mixPublicKey`, in the form the
    ## `--mix-node` argument takes. A mix pool below `MinMixPoolSize` can build
    ## no path, so a preset that turns mix on seeds one rather than leaving the
    ## node to wait for discovery. Names, not literal addresses: `mountMix`
    ## resolves them, and a name survives a fleet node moving hosts.
  p2pReliability*: bool
