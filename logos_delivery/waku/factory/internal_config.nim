...
proc enrConfiguration*(
  ...
  ## Build an ENR from the NetConfig's ENR fields. Multiaddrs trim from
  ## the tail to fit, and seqNum must exceed the previous record's.
...
proc networkConfiguration*(
  ...
  ## Only extip and the dns4 name vouch for an external IP here. The
  ## NATService's mappings never enter this derivation.
  var extIp =
  ...
  let
    ...
    ## With extip or dns4 the operator vouches for the endpoint. Without
    ## a manual port config the external ports are the bind ports.
    extPort =
