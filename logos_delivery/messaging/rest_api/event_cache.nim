...
## it. A generous overflow cap bounds memory if nobody polls. Overflow is never
## silent: every eviction counts towards the next poll's `dropped` figure and a
## metric, and each received record carries a monotonic `seq`, so a gap between
## polls is a loss the client can see. All access happens on the single chronos
## event loop (broker listeners + REST handlers), so the synchronous (no-await)
## ops below need no locking.
...
proc recordReceived*(
  ...
  ## Every drop is counted for the next poll and the metric; the record's
  ## `seq` lets a client see the gap.
...
proc pollAllSend*(
  ...
  ## with the number of request ids evicted unpolled since the previous poll.
...
proc pollReceived*(
  ...
  ## Return buffered received messages (oldest first) and clear
  ## (evict-after-poll), with the number of records evicted unpolled since the
  ## previous poll.
