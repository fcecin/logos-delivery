...
The reply of the exit node travels a return path that ends at the sender. The mix node before the sender delivers the reply by dialing the sender's address, and libp2p reuses an existing connection before it dials. A node that nothing can dial (a phone behind a carrier NAT) receives a reply only over a connection it opened itself. So the node keeps a connection to one mix node of the pool, the reply anchor, and asks the library to put the anchor as the hop that delivers every reply. The node picks the anchor at random when it first sends, keeps it while the connection lives, and picks another when the connection drops or when the anchor is the exit node of a send. Without an anchor (no pool node accepts a connection), the send goes out with a random last hop, and the reply arrives only by luck.

The anchor learns the node's address, which the node's first hops learn anyway, and the timing of replies. It does not see the forward packet of the same message unless the random forward path happens to start at it. It does not see the message, the exit node, or the destination. The anchor is long-lived where the first hops rotate per packet; a rotation of the anchor per interval or per count of messages is a follow-up. The node dials one anchor candidate per publish, for at most 3 s, so a lost anchor costs an attempt that much and no more; a candidate that did not answer is skipped until every candidate has failed.
