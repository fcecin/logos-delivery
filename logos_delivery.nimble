...
requires "nim >= 2.2.4",
  ...
  "libp2p == 2.2.1",
  "eth",
  # nat_traversal stays in the graph through libp2p, which links the
  # miniupnpc and libnatpmp static libs. Nat.mk and the iOS steps stay.
  "dnsdisc",
  "dnsclient",
  "httputils >= 0.4.1",
  "websock >= 0.3.0",
  ...
  "zlib",
  # Debug & Testing
  # 0.8.2 imports results in its fuzzing tool and breaks the Windows
  # --localdeps install. 0.8.1 is the revision that master locks.
  "testutils == 0.8.1",
  "unittest2"
...
# Exact pin: libp2p only sets a floor, and a re-solve floats to the
# newest lsquic release and rewrites the lock.
