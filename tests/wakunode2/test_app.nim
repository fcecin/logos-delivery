...
suite "Wakunode2 - Waku initialization":
  ...
  test "production service composition carries NATService and IdentifyPusher":
    ## Full path from conf to switch services: the NAT config is wired
    ## through and the builder services survive the append.
    var conf = defaultTestWakuConf()
  ...
  test "a post-start commit reaches the ENR through the production hook":
    ## Deleting the onCommittedAddresses callback, or setting it after
    ## the first refresh, fails this.
    var conf = defaultTestWakuConf()
    var waku = (waitFor Waku.new(conf)).valueOr:
      raiseAssert error
    (waitFor waku.start()).isOkOr:
