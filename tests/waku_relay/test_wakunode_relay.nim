...
suite "WakuNode - Relay":
  ...
  asyncTest "Bad peers with low reputation are disconnected":
    # The bad-peer scoring under test is transport agnostic. TCP teardown
    # is deterministic enough to assert on connection counts.
    let nodes =
    ...
    # The disconnect sweep runs on a score-decay tick and the kicked side
    # sees the closes asynchronously. Poll for the terminal state.
