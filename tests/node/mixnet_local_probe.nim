...
proc main() {.async.} =
  ...
  # what a NATed node effectively does. buildSurb embeds the sender's announced
  # multiaddr as the reply's last hop; a fresh dial to it fails, but nim-libp2p
  # reuses a live connection by peer id, and every core holds one because the
  # sender dialed it. Expected: OK. `isolated` below removes that guarantee.
  ...
  # --- relay subscriptions ----------------------------------------------------
  # One handler per node, labelled: the line says where the message arrived.
  ...
  proc subscribeProbe(node: WakuNode, label: string) =
    proc handler(topic: PubsubTopic, msg: WakuMessage): Future[void] {.async.} =
      let p = string.fromBytes(msg.payload)
      if p.startsWith(runId):
        forwardSeen.inc()
        echo ts(), " AXIOM-FORWARD: message on relay at ", label, ", payload=", p

    node.subscribe((kind: PubsubSub, topic: shard), handler).isOkOr:
      echo "FATAL subscribe ", label, ": ", error
      quit(QuitFailure)

  for i in 0 ..< NumCore:
    subscribeProbe(nodes[i], "core " & $i)
  subscribeProbe(sender, "sender")
