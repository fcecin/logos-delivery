...
suite "Messaging REST API":
  asyncTest "subscribe / unsubscribe / send endpoints respond":
    ...
    # A malformed content topic is the caller's error: 400, not 500, on every
    # route that takes one; so is a generation the sharding cannot resolve.
  ...
  asyncTest "without autosharding, subscribe, unsubscribe and send answer 503":
    ## No preset and no shard count: static sharding, so content topics cannot
    ## resolve to a shard. The answer names the flags instead of a 500.
    var node: LogosDelivery
    lockNewGlobalBrokerContext:
      node = (
