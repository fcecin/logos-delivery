...
diag-nimble: | install-nimble
	@echo "MAKE_VERSION=$(MAKE_VERSION) MAKEFLAGS=$(MAKEFLAGS) HOME=$(HOME) CURDIR=$(CURDIR)"
	@echo "NIMBLE=$(NIMBLE)"
	@ls -la "$(NIMBLE_TOOLDIR)"
	@echo "recipe PATH=$$PATH"
	@echo "shell command -v nimble: $$(command -v nimble)"
	@if [ -x "$(NIMBLE)" ]; then echo "shell: NIMBLE is executable"; else echo "shell: NIMBLE is NOT executable"; fi
	nimble --version
	sh -c 'nimble --version'
	sh -c '"$(NIMBLE)" --version'
	$(NIMBLE) --version
