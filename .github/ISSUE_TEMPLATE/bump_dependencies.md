...
- [ ] Update nimble dependencies
  ...
  2. Run `make nimble`, then `"$(make -s print-nimble-path)/nimble" lock --localdeps -y --useSystemNim`, which is the pinned Nimble. `nimble lock` is the canonical lock writer; `tools/sync-nimble-lock.sh` predates Nimble 0.24 and is legacy
